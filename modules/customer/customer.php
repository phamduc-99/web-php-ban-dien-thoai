...
if(isset($_POST['name']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['add'])){
    ...
    while($row = mysqli_fetch_assoc($query)){
        ...
        $total_amount = $quantity*$row['prd_price'];
        $emailHTML .= '
        <tr>
        <td width="70%">'.$row['prd_name'].'</td>
        <td width="10%">'.$quantity.'</td>
        <td width="20%">'.$total_amount.' đ</td>
        </tr>
        ';
    }
    $emailHTML .= '
...
try {
    ...
    $mail->Username   = 'phamduc99yn@gmail.com';                     // SMTP username
    $mail->Password   = 'ffmkimbxqdjeruvb';                               // SMTP password
    $mail->SMTPSecure = 'TLS';         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` also accepted
    $mail->Port       = 587;                                    // TCP port to connect to
    //Recipients
    $mail->setFrom('phamduc99yn@gmail.com', 'Shop');
    $mail->addAddress($email, 'Khách hàng');     // Add a recipient
    // $mail->addAddress('ellen@example.com');               // Name is optional
    // $mail->addReplyTo('info@example.com', 'Information');
    $mail->addCC('phamduc99yn@gmail.com');
    ...
    $mail->Subject = 'Xác nhận đơn hàng từ Shop';
    $mail->Body    = $emailHTML;
